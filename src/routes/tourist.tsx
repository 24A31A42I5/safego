        ...
        <CardContent className="space-y-3">
          ...
          {geoError && (
            <p className="text-xs text-muted-foreground">
              Location access denied. Enable in browser to use the map fully.
            </p>
          )}
          {location && (
            <p className="text-xs text-muted-foreground">
...
function LostReportDialog() {
  ...
  const submit = async () => {
    ...
    if (file) {
      ...
      const { data } = await supabase.storage.from("lost-photos").createSignedUrl(
        path,
        60 * 60 * 24 * 365
      );
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Provide details so authorities can begin a search.
          </DialogDescription>
        </DialogHeader>
